...
EdgeAgentTwinRequestThrottleInSeconds=10
...
devices=($(az iot hub query --hub-name $iothub_name --query-command "SELECT * FROM devices WHERE tags.environment = '$environment'" | jq -r .[].deviceId))

if [ ${#devices[*]} -eq 0 ]
then
        echo "No $environment Devices found in $iothub_name"
        return 1
else
        echo "Number of $environment Devices found in $iothub_name: ${#devices[*]}"
fi
...
validateDevicesConnectedToIoTHub(){
echo "Validating that all $environment devices are curently connected to the iot hub..."
...
validateDevicesAppliedDeployment(){
echo "Validating that all $environment devices have applied the $deployment_name deployment..."
for device in ${devices[*]}
do
        getEdgeAgentTwin
...
validateDevicesModulesRunning(){
echo "Validating that all $environment devices configured modules are running..."
for device in ${devices[*]}
do
        getEdgeAgentTwin
